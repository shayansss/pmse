...
Sarrays   = odb.steps['static'].frames[-1].fieldOutputs['S'].getSubset(region = odbInstance, position = ELEMENT_NODAL).values
region = [i.nodeLabel for i in Sarrays]

name = 'temp_%s'%(np.random.random())


newFieldOutput = odb.steps['static'].frames[0].FieldOutput(name = name, description = '', type = SCALAR)
